...
make deploy-certs      # Upload generated SSL certs to the server
```
...
| `local_file.*` | Cert PEMs written to `.certs/` |

After `terraform apply`, run `make deploy-certs` to SCP the new certs to the server and reload Nginx.

> **Note**: Terraform state is stored locally in `terraform.tfstate`. Keep a backup.
...
1. Update `terraform.tfvars` with your API tokens
2. Run `make terraform-apply` — creates the Linode with cloud-init (Nginx, PHP 8.4, Composer, Node.js)
